...
vec3 rayMarch(Ray cameraRay) {
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        ...
        mat3 rotMatLight = rotationAroundAxis(vec3(point.x, 1.0, -point.z), u_time * 2.5);
        point = point * rotMat;
        ...
        if (dist < HIT_EPSILON) {
            ...
            float spec = pow(max(dot(H, normal), 0.0), 25.0);

            float fresnel = pow(1.0 - dot(viewDir, normal), 36.0);

            float lighting = diffuse + spec;
            vec3 color = vec3(
                    smoothstep(0.0, 1.0, point.x),
                    1.0,
                    smoothstep(0.1, 0.8, fresnel)) + lighting;
            return color;
        }
        ...
        if (t > FAR_CLIP) {
            ...
            return mix(fogColor, vec3(0.2, 0.2, 0.2), fogAmount);
        }
        t += dist;
    }
    vec3 fogColor = vec3(t / 30.0, 1., 1.0);
    float fogAmount = 1.0 - exp(-t * 0.2);
    return mix(vec3(1.0, 0.25, .20), vec3(0.0, 0.1, 0.1), fogAmount);
}
