...
float distFunc(vec3 p, out int matID) {
    ...
    float d1 = sdBox(p - vec3(-5.0, 0.0, 3.0), vec3(1.0, 5.0, 1.0));
    float d2 = sdBox(p - vec3(0.0, 0.0, 2.5), vec3(1.0, 5.0, 1.0));
    float sphere = sdSphere(p - getLightPos(), 2.5);
    float d = d0;
    matID = 0;
...
float distOnlyNoLight(vec3 p) {
    float d0 = sdPlane(p, vec3(0, 1, 0), -0.5);
    float d1 = sdBox(p - vec3(-5.0, 0.0, 3.0), vec3(1.0, 2.0, 1.0));
    float d2 = sdBox(p - vec3(0.0, 0.0, 2.5), vec3(1.0, 5.0, 1.0));
