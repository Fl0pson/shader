...
void main() {
    ...
    vec2 uv_rot_1 = rotate(uv, step(0.5, length(uv) * u_time) * u_time * sign);
    ...
    float curve4 = draw_spiral(uv_rot_1, arm_thickness + random2d(uv), angle_scale, angle_scale * TWOPI, sharpness);
    float wave = sin_wave(.25, acos(random2d(uv)), cos(length(uv)), 0.15);
    float wave2 = cos_wave(.75, asin(random2d(uv_rot_1)), cos(length(uv)), 0.55);
    vec4 color = vec4(0.0);
    color = mix(color,
            vec4(
                wave,
            ...
            1.0 - smoothstep(0.5, 1.0, curve * wave));
